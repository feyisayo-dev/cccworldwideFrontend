...
<template>
        ...
        <VRow no-gutters>
          <VCol cols="8">
            <VCardText>
              <h6 class="text-lg text-no-wrap font-weight-medium">
                Hello {{ genderTitle() }} {{ userData.fname }} {{ userData.mname }} {{ userData.sname }} 🎉
              </h6>
              <p class="mb-2">
                {{ userData.Title }}
              </p>
          ...
          <VCol cols="4">
            <VCardText class="pb-0 px-0 position-relative h-100">
              <VImg :src="userData.avatar || congoImg" height="147" class="congo-john-img w-100" />
            </VCardText>
          </VCol>
        </VRow>
      </VCard>
  ...
  <EditMemberDialog v-model:isDialogVisible="isEditDialogVisible" :user-data="userData" />
</template>
